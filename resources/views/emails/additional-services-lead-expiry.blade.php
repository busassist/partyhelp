{{-- Additional Services / Lead Expiry – +72h customer email. Inline styles for email clients. --}}
@php
    ...
    $headerText = $header_text ?? "Your lead window has closed – here's what's next";
    $expiryIntroText = $expiry_intro_text ?? 'The 72-hour window for venues to purchase your lead has now closed.';
    $additionalServicesText = $additional_services_text ?? 'If you would like more venue options or help finalising your booking, we can still assist.';
    $ctaText = $cta_text ?? 'Contact us for more venue options or booking support.';
    $closingText = $closing_text ?? 'Thank you for using Partyhelp.';
@endphp
...
<p style="font-size:15px;color:#cbd5e1;line-height:1.6;margin:0 0 16px;">{!! $expiryIntroText !!}</p>
<p style="font-size:15px;color:#cbd5e1;line-height:1.6;margin:0 0 16px;">{!! $additionalServicesText !!}</p>
