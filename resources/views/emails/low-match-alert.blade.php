{{-- Low-Match Alert – Admin, when fewer than 10 venues match a lead. PRD 9.1. Inline styles for email clients. --}}
@php
    $logoUrl = asset('images/brand/ph-logo-white.png');
    $headerText = $header_text ?? 'Low-match alert';
    $introText = $intro_text ?? 'Fewer than 10 venues matched this lead. You may want to review matching criteria or contact the customer.';
    $viewLeadLabel = $view_lead_label ?? 'View lead in dashboard';
@endphp
...
<p style="font-size:14px;color:#cbd5e1;margin:0 0 8px;"><strong style="color:#a855f7;">Suburb:</strong> {{ $suburb ?? '—' }}</p>
...
<p style="font-size:14px;color:#f59e0b;margin:16px 0 0;"><strong>Match count: {{ $matchCount ?? '—' }} venues</strong></p>
</td></tr></table>
<p style="text-align:center;margin:24px 0;"><a href="{{ $dashboardLeadUrl ?? '#' }}" style="display:inline-block;background:#7c3aed;color:#fff!important;font-size:15px;font-weight:600;padding:12px 24px;text-decoration:none;border-radius:8px;">{!! $viewLeadLabel !!}</a></p>
</td></tr>
