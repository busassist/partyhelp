{{-- Function Pack – Venue, when they purchase a lead. Download link. Inline styles for email clients. --}}
@php
    $logoUrl = asset('images/brand/ph-logo-white.png');
    $headerText = $header_text ?? 'Your function pack is ready to download';
    $introText = $intro_text ?? 'You have purchased a lead. Download the function pack below to access the full customer details and materials.';
    $downloadButtonLabel = $download_button_label ?? 'Download function pack';
    $closingText = $closing_text ?? 'The download link will expire after 30 days. Contact us if you need assistance.';
@endphp
...
<p style="text-align:center;margin:0 0 24px;"><a href="{{ $downloadUrl ?? '#' }}" style="display:inline-block;background:#7c3aed;color:#fff!important;font-size:16px;font-weight:600;padding:14px 28px;text-decoration:none;border-radius:8px;">{!! $downloadButtonLabel !!}</a></p>
@if($expiryNote ?? null)
<p style="font-size:14px;color:#94a3b8;margin:0 0 24px;">{!! $expiryNote !!}</p>
@endif
