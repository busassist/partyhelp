<div style="display: flex; flex-direction: column; height: 100%; min-height: 0; background: white; font-family: 'Instrument Sans', ui-sans-serif, system-ui, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol', 'Noto Color Emoji';" x-data="{ tab: 'library' }" x-on:media-modal-reset.window="tab = 'library'" @media-upload-complete.window="tab = 'library'">
    ...
    <div style="flex: 1; min-height: 0; overflow-y: auto; padding: 2rem; background: white;" class="media-modal-scroll">
        <div x-show="tab === 'library'" x-transition:enter.opacity.duration.150ms>
            <div class="media-modal-grid">
                @forelse($this->mediaItems as $media)
                    ...
                    @endphp
                    <div class="media-modal-card" style="display: flex; flex-direction: column; gap: 1rem; padding: 1rem; background: white; border: 1px solid {{ $this->themeColors['borderLight'] }}; border-radius: 0.75rem; text-align: left; transition: box-shadow 0.2s, border-color 0.2s;" onmouseover="this.style.boxShadow='0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -2px rgba(0,0,0,0.1)'; this.style.borderColor='rgba(124,58,237,0.4)'" onmouseout="this.style.boxShadow='none'; this.style.borderColor='{{ $this->themeColors['borderLight'] }}'">
                        <div class="media-modal-card-image-wrap" style="position: relative; width: 100%; aspect-ratio: 4/3; overflow: hidden; border-radius: 0.5rem; background: {{ $this->themeColors['bgInput'] }};">
                            ...
                            <div class="media-modal-card-overlay" style="position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; gap: 0.5rem; background: rgba(0,0,0,0.4);">
                                <button type="button" wire:click="selectImage('{{ $media->file_path }}')" class="media-modal-card-btn media-modal-card-btn-select" style="padding: 0.375rem 0.75rem; font-size: 0.75rem; font-weight: 600; color: white; background: {{ $this->themeColors['primary'] }}; border: none; border-radius: 0.375rem; cursor: pointer; box-shadow: 0 1px 2px rgba(0,0,0,0.2);">Select</button>
                                <button type="button" wire:click="deleteImage({{ $media->id }})" wire:confirm="Delete this image from the library?" class="media-modal-card-btn media-modal-card-btn-delete" style="padding: 0.375rem 0.75rem; font-size: 0.75rem; font-weight: 600; color: white; background: #dc2626; border: none; border-radius: 0.375rem; cursor: pointer; box-shadow: 0 1px 2px rgba(0,0,0,0.2);">Delete</button>
                            </div>
                        </div>
        ...
        <div x-show="tab === 'upload'" x-transition:enter.opacity.duration.150ms x-cloak wire:key="upload-tab-{{ $uploadIteration }}-{{ $errors->has('uploadedFile') ? 1 : 0 }}">
            <div x-data="{ dragging: false, uploading: false, drop(e) { this.dragging = false; const files = e.dataTransfer?.files; if (files?.length) { this.uploading = true; $wire.upload('uploadedFile', files[0]); } }, dragOver(e) { e.preventDefault(); this.dragging = true; }, dragLeave() { this.dragging = false; } }" x-on:dragover.prevent="dragOver($event)" x-on:dragleave.prevent="dragLeave()" x-on:drop.prevent="drop($event)" x-on:media-upload-complete.window="uploading = false" style="display: grid; grid-template-columns: 1fr; grid-template-rows: minmax(400px, 1fr); width: 100%; border: 2px dashed {{ $this->themeColors['borderDark'] }}; border-radius: 0.5rem; padding: 3rem 2rem; text-align: center; background: transparent; min-height: 400px; transition: border-color 0.2s, background 0.2s;" :style="dragging ? 'border-color: {{ $this->themeColors['primary'] }}; background: rgba(124,58,237,0.04);' : ''">
                <div style="grid-area: 1/1; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                    <input type="file" wire:model="uploadedFile" accept="image/*" capture="environment" id="media-upload-input-{{ $uploadIteration }}" style="display: none;" @change="uploading = true" />
                    <label for="media-upload-input-{{ $uploadIteration }}" style="cursor: pointer; display: flex; flex-direction: column; align-items: center; text-align: center;" x-show="!uploading" x-cloak>
                        <div style="width: 4rem; height: 4rem; border-radius: 9999px; background: {{ $this->themeColors['bgInput'] }}; display: flex; align-items: center; justify-content: center; margin-bottom: 1rem; margin-left: auto; margin-right: auto;">
                        ...
                        <p style="font-size: 0.9375rem; font-weight: 600; color: {{ $this->themeColors['textDark'] }}; margin: 0 0 0.25rem; text-align: center;">Drag and drop an image here</p>
                        <p style="font-size: 0.875rem; color: {{ $this->themeColors['textMuted'] }}; margin: 0 0 1rem; text-align: center;">or click the button below to browse your files</p>
                        <span style="display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem; padding: 0.5rem 1.25rem; font-size: 0.875rem; font-weight: 600; color: white; background: {{ $this->themeColors['primary'] }}; border-radius: 0.5rem; cursor: pointer; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                        ...
                        <p style="font-size: 0.75rem; color: {{ $this->themeColors['textLight'] }}; margin: 0.75rem 0 0; text-align: center;">Supported formats: JPEG, PNG, GIF, WEBP • Max size: 10MB</p>
                    </label>
                </div>
                <div x-show="uploading" x-cloak class="media-upload-loading" style="grid-area: 1/1; place-self: stretch; display: flex; flex-direction: column; align-items: center; justify-content: center; background: rgba(255,255,255,0.98); border-radius: 0.5rem; z-index: 10; width: 100%; text-align: center;">
                    <div style="width: 3rem; height: 3rem; border: 3px solid {{ $this->themeColors['borderMed'] }}; border-top-color: {{ $this->themeColors['primary'] }}; border-radius: 50%; animation: media-upload-spin 0.8s linear infinite; flex-shrink: 0; margin-left: auto; margin-right: auto;"></div>
                    <p style="margin: 1rem 0 0; font-size: 0.9375rem; font-weight: 600; color: {{ $this->themeColors['textDark'] }}; text-align: center;">Uploading your image...</p>
                    <p style="margin: 0.25rem 0 0; font-size: 0.8125rem; color: {{ $this->themeColors['textMuted'] }}; text-align: center;">Please wait</p>
                </div>
                @error('uploadedFile')
